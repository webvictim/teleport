...
func (c *CredentialsClient) SSHAgentSSOLogin(ctx context.Context, connectorID string, pubKey []byte, ttl time.Duration, protocol string, compatibility string) (*auth.SSHLoginResponse, error) {
	...
	c.url.Path = "/web/msg/error/login_failed"
	redirectErrorURL := c.url.String()
	c.url.Path = "/web/msg/info/login_success"
	redirectSuccessURL := c.url.String()
	...
	out, err := c.clt.PostJSON(ctx, c.clt.Endpoint("webapi", protocol, "login", "console"), SSOLoginConsoleReq{
